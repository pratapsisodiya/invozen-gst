...
export async function downloadQuotationPdf(
  ...
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  doc.text(`#${q.quotationNumber}`, pageW - margin, 18, { align: 'right' })
  ...
  doc.setFontSize(9)
  doc.setTextColor(60, 60, 60)
  doc.text(q.customerSnapshot.name, pageW / 2 + 5, y)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.setTextColor(80, 80, 80)
  doc.text(`GSTIN: ${q.customerSnapshot.gstin || 'Unregistered'}`, pageW / 2 + 5, y + 5)
  doc.text(q.customerSnapshot.state, pageW / 2 + 5, y + 10)
  ...
  const metaItems = [
    ...
    ['Supply', q.supplyType === 'intra' ? 'Intra-State' : 'Inter-State'],
  ]
  ...
  addRow('Taxable Value', q.taxableValue.toLocaleString('en-IN', { maximumFractionDigits: 2 }))
  if (q.cgstTotal > 0) addRow('CGST', q.cgstTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 }))
  if (q.sgstTotal > 0) addRow('SGST', q.sgstTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 }))
  if (q.igstTotal > 0) addRow('IGST', q.igstTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 }))

  y += 2
  doc.setDrawColor(13, 148, 136)
  doc.line(totalsX - 5, y, margin + contentW, y)
  y += 3
  addRow('QUOTATION TOTAL', q.grandTotal.toLocaleString('en-IN'), true)
  ...
  if (q.notes) {
    ...
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(80, 80, 80)
    doc.text(doc.splitTextToSize(q.notes, contentW), margin, y + 5)
  }
  ...
  doc.setFontSize(7)
  doc.setFont('helvetica', 'normal')
  doc.text('This is a Quotation / Proforma. This is NOT a Tax Invoice under the CGST Act, 2017.', pageW / 2, 290, { align: 'center' })
