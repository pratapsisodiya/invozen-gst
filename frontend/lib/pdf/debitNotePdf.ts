...
export async function downloadDebitNotePdf(
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text('DEBIT NOTE', pageW - margin, 12, { align: 'right' })
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  doc.text(`#${dn.debitNoteNumber}`, pageW - margin, 18, { align: 'right' })
  ...
  doc.text('TO (VENDOR)', pageW / 2 + 5, y)
  ...
  doc.setFontSize(9)
  doc.setTextColor(60, 60, 60)
  doc.text(dn.vendorSnapshot.name, pageW / 2 + 5, y)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.setTextColor(80, 80, 80)
  doc.text(`GSTIN: ${dn.vendorSnapshot.gstin || 'Unregistered'}`, pageW / 2 + 5, y + 5)
  doc.text(dn.vendorSnapshot.state, pageW / 2 + 5, y + 10)
  ...
  doc.setFontSize(8)
  const metaItems = [
    ['Date', new Date(dn.createdAt).toLocaleDateString('en-IN')],
    ['Linked Purchase', dn.linkedPurchaseNumber || 'N/A'],
  ...
  addRow('Taxable Value', dn.taxableValue.toLocaleString('en-IN', { maximumFractionDigits: 2 }))
  if (dn.cgstTotal > 0) addRow('CGST', dn.cgstTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 }))
  if (dn.sgstTotal > 0) addRow('SGST', dn.sgstTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 }))
  if (dn.igstTotal > 0) addRow('IGST', dn.igstTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 }))

  y += 2
  doc.setDrawColor(13, 148, 136)
  doc.line(totalsX - 5, y, margin + contentW, y)
  y += 3
  addRow('DEBIT NOTE TOTAL', dn.grandTotal.toLocaleString('en-IN'), true)
  ...
  doc.setFontSize(7)
  doc.setFont('helvetica', 'normal')
  doc.text('This is a GST Debit Note issued under Section 34 of the CGST Act, 2017.', pageW / 2, 290, { align: 'center' })
