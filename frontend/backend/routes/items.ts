import { Router } from 'express'
import { requireAuth } from '../middleware/auth'
import type { AuthRequest } from '../middleware/auth'
import { prisma } from '../lib/prisma'
import { toJson } from '../lib/prisma'
import { ok, created, notFound, badRequest } from '../lib/response'

const router = Router()
router.use(requireAuth)

router.get('/', async (req, res, next) => {
  try {
    const userId = (req as unknown as AuthRequest).userId
    const { search, type, active } = req.query as Record<string, string>

    const rows = await prisma.item.findMany({
      where: {
        userId,
        ...(type ? { type } : {}),
        ...(active !== undefined ? { isActive: active === 'true' } : {}),
        ...(search ? { name: { contains: search } } : {}),
      },
      orderBy: { createdAt: 'desc' },
    })

    ok(res, rows.map((r) => r.data))
  } catch (err) { next(err) }
})

router.post('/', async (req, res, next) => {
  try {
    const userId = (req as unknown as AuthRequest).userId
    const body = req.body as Record<string, unknown>
    if (!body['id'] || !body['name']) return badRequest(res, 'id and name are required')

    const row = await prisma.item.create({
      data: {
        id: body['id'] as string,
        userId,
        name: body['name'] as string,
        type: (body['type'] as string) || 'product',
        isActive: (body['isActive'] as boolean) ?? true,
        data: toJson(body),
      },
    })
    created(res, row.data)
  } catch (err) { next(err) }
})

router.get('/:id', async (req, res, next) => {
  try {
    const userId = (req as unknown as AuthRequest).userId
    const row = await prisma.item.findFirst({ where: { id: req.params['id'], userId } })
    if (!row) return notFound(res)
    ok(res, row.data)
  } catch (err) { next(err) }
})

router.put('/:id', async (req, res, next) => {
  try {
    const userId = (req as unknown as AuthRequest).userId
    const existing = await prisma.item.findFirst({ where: { id: req.params['id'], userId } })
    if (!existing) return notFound(res)

    const merged = { ...(existing.data as object), ...req.body }
    const row = await prisma.item.update({
      where: { id: req.params['id'] },
      data: {
        name: (merged as Record<string, unknown>)['name'] as string || existing.name,
        type: (merged as Record<string, unknown>)['type'] as string || existing.type,
        isActive: (merged as Record<string, unknown>)['isActive'] as boolean ?? existing.isActive,
        data: toJson(merged),
        updatedAt: new Date(),
      },
    })
    ok(res, row.data)
  } catch (err) { next(err) }
})

router.delete('/:id', async (req, res, next) => {
  try {
    const userId = (req as unknown as AuthRequest).userId
    const existing = await prisma.item.findFirst({ where: { id: req.params['id'], userId } })
    if (!existing) return notFound(res)
    await prisma.item.delete({ where: { id: req.params['id'] } })
    ok(res, { deleted: true })
  } catch (err) { next(err) }
})

export default router
