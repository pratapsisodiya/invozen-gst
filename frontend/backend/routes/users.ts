...
import { requireAuth, clerkClient } from '../middleware/auth'
import type { AuthRequest } from '../middleware/auth'
import { prisma } from '../lib/prisma'
import { toJson } from '../lib/prisma'
import { ok, created, notFound, badRequest, forbidden } from '../lib/response'
...
router.post('/invite', async (req, res, next) => {
  try {
    ...
    if (err instanceof z.ZodError) {
      return badRequest(res, err.issues.map(e => `${e.path.join('.')}: ${e.message}`).join(', '))
    }
    next(err)
  }
})
